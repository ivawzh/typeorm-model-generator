...
function validateConfig(options) {
    ...
    else if (options.generationOptions.activeRecord &&
        options.generationOptions.extendAbstractClass) {
        TomgUtils.LogError("Typeorm cannot use ActiveRecord and extend-abstract-class at the same time.", false);
...
async function useInquirer(options) {
    ...
    if (customizeGeneration) {
        ...
        const customizations = (await inquirer.prompt([
            {
                choices: [
                    ...
                    },
                    {
                        ...
                        checked: options.generationOptions.skipRelationships,
                    },
                    {
                        name: "Generated models extend a custom abstract class",
                        value: "extendAbstractClass",
                        checked: options.generationOptions.extendAbstractClass,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        checked: options.generationOptions.exportAbstractClass,
                    },
                ],
        ...
        options.generationOptions.skipRelationships = customizations.includes("skipRelationships");
        if (customizations.includes("extendAbstractClass")) {
            ...
            if (extendAbstractClass && extendAbstractClass !== "") {
                const resultsAbsolutePath = path.join(process.cwd(), options.generationOptions.resultsPath);
                const abstractClassAbsolutePath = path.join(process.cwd(), options.generationOptions.resultsPath);
                const relativePath = path.relative(abstractClassAbsolutePath, resultsAbsolutePath);
        ...
        options.generationOptions.exportType = customizations.includes("defaultExport")
            ? "default"
            : "named";
        options.generationOptions.exportAbstractClass = customizations.includes("exportAbstractClass");
